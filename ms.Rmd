...
Such a model can be quickly and easily re-constructed as a `zoon` workflow using modules which are already available in the software repository:
...
The resulting workflow contains all the code required to re-run the workflow, and the results of executing each module.
...
Next, we update the workflow to produce an interactive map enabling anyone to inspect the data and  predictions on a zoomable map, and to inspect the response curves of the fitted model:

```{r, eval=FALSE}
carruthers_et_al_interactive <- ReRunWorkflow(
    carruthers_et_al,
    output = Chain(InteractiveMap, ResponseCurve))
```

![The interactive map module overlay the raw data and predicted distribution, allowing users to interactively explore their results.](./figs/interactive_map.png)

<!-- add response curves output here -->

`zoon` users can inspect existing workflows using these and a range of other *output* modules, or create and share their own output modules.


#### Example 2. Evaluating the MaxLike model
