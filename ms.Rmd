...
zoon differs from these packages in that zoon modules are drawn from an open repository that any SDM user can contribute to, and zoon makes it easy for SDM users who are not expert R developers to contribute.
zoon's focus on reproducibile and modifiable workflows also shares similarities with general-purpose workflow systems such as Taverna [@] and its extensions for biodiversity research via the BIOVEL system [@DeGiovanni2015,@DeGiovanni2016].
However unlike these workflow environments in which users may embed R code, zoon embeds an SDM-centric workflow system within R.
zoon therefore provides the benefits of workflow systems to SDM users without them needing to learn complex workflow systems.
