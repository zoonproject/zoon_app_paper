...
#### Example 2. Building a point process model module

Recent SDM research has identified an equivalence between MaxEnt and Poisson point process models (PPMs) [] and noted that PPMs can be fitted using Poisson modelling software [Warton & Renner, Renner et al.]. This means that other modelling approaches can be fitted as PPMs, after a bit of data preparation. We illustrate how to build a `zoon` model module by defining a module to fit a PPM using a Generalized Additive Model (GAM). 

The code for our model is as follows (see Renner et al., 2014 for details of this approach):
...
The only complicated aspect of this code is construction of a `zoonModel` object, which contains code to make predictions from the model in a standardised way. The other module types do not require this step, so are simpler to implement. The code also meets the minimal requirements of a *model* module, accepting the required argument `.df` and returning the `zoonModel` object.

To convert this code into a ZOON module, we need to write it to a standalone file with the required metadata. the `zoon` function `BuildModule` helps with this step, and can also run checks to make sure we got everything right:

```{r buildmodule, eval = FALSE} 
BuildModule(PPM_GAM, ...)
```

This module can now be used in a `zoon` workflow and shared with others. To upload the module to the `zoon` modules repository, [TBD].
