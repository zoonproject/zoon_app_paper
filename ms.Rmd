...
The following workflow demonstrates use of the chains and lists to fit three different SDMs to a presence-background dataset for a UK mosquito species, after standardising the covariates.
The results of the three models are then returned in Shiny app allowing the user to interactively explore the occurrence and covariate data, model summaries and prediction maps.
...
MaxEnt is one of the most popular models for presence-only SDM [@Ahmed2015], in part because it performed very well in the comprehensive comparison of @Elith2006.
Unlike most other models use in SDM, MaxEnt considers occurrence records as arising from some probability distribution over space and attempts to model that distribution, subject to a 'maximum entropy' constraint [@Phillips2006].
Subsequent research has shown that although it approaches the problem from a diffent perspective, the model MaxEnt fits is in fact equivalent to Poisson point process model [@Renner?].
Moreover, so long as the number of background points large, this model is essentially the same as a logistic regression model [@?].
Given MaxEnt's treatment of the data is not fundamentally different than other common models, @Renner2013 and others have suggested that MaxEnt's predictive power is instead due to the array of features it constructs, and the fact that it applies regularisation when fitting the model.
Whilst the various features (candidate covariate transformations) increase the potential flexibility of the model, regularisation acts to prevent MaxEnt from overfitting to the data.

Here we illustrate how zoon can facilitate SDM methodological comparisons by fitting MaxEnt models with and without regularisation and feature construction.
We fit MaxEnt with both the usual java implementation, and an implementation recently provided in the `maxnet` R package [@maxnet] which approximates the full MaxEnt model via a regularised logistic regression.

The following zoon workflow fits five different models to a presence-background dataset, plots a prediciton map generated by each and evaluates the predictive performance of each model against a presence-absence dataset.
The presence-only and presence-absence data and covariates are for the Caroline wren in the USA, as analysed by @Royle2012 and @Merow2014, and distributed via the `maxlike` R package [@maxlike].
The five models compared are: the java implementation of MaxEnt, the `maxnet` implementation (via the `MaxNet` zoon module), the `maxnet` version without regularisation, without features, and without regularisation or features. 
As in @Merow2014, each model is trained on the same randomly selected subset of 500 occurrence records, with 10,000 randomly selected background records.
Statistics of the predictive performance of each fitted model are then calculated against the full presence-absence dataset, and prediction maps plotted in Figure 3.

```{r maxnet_comparison, dpi = 600, fig.show = "hide", eval = TRUE}
maxnet_comparison <- workflow(
  ...
  output     = Chain(PerformanceMeasures,
                     PrintMap(points = FALSE)))
```

```{r maxnet_plotting, eval = TRUE, echo = FALSE, dpi = 600, fig.show = "hide", fig.height = 9, fig.width = 1.5}
...
```{r maxnet_plots, cache = FALSE, echo = FALSE, out.width = "150px", fig.cap = ""}
# knitr::include_graphics("figs/maxnet_plotting-1.png")
```

```{r maxent_aucs, eval = TRUE, echo = FALSE}
# AUCs, not quite sure what to do with these - embed them in the text somehow?
perflist <- maxnet_comparison$report[seq(1, 10, by = 2)]
AUCs <- sapply(perflist, function(x) x$auc)
names(AUCs) <- c('MaxEnt',
                 'MaxNet',
                 'MaxNet (no regularization)',
                 'MaxNet (no features)',
                 'MaxNet (no regularization or features)')
...
cat('.')
```
Readers interested in other statistics, viewing response curves or adjusting the calidation method can easily reproduce and modify the analysis either by re-running the above code block or by downloading the workflow object from: <URL TBD>
