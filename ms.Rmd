...
Examples of subject specific frameworks are found within the Bioinformatics community, for example the Bioconductor infrastructure that uses R (@Gentleman2004, https://www.bioconductor.org/), and BioJS, a repository for JavaScript components (@Corpas2014, https://www.biojs.net/).
...
In terms of robustness, the CHASTE project for cardiac modelling (@Mirams2013, http://www.cs.ox.ac.uk/chaste/) has developed into a model repository that offers community benchmarks for the last 50 years of cardiac modelling [@Cooper2016].
...
```{r feng_papes_plots, cache = FALSE, echo = FALSE, out.width = "450px", fig.cap = "Outputs of the workflow objects `FengPapes' and `FengPapesUpdate'. A) Map of the presence data (red), background data (black) and the MaxEnt relative probability of occurrence generated by the `PrintMap' module in the workflow `FengPapes'. White areas are masked due to being in the sea or outside the MESS mask. B) A screenshot of the interactive map produced by the `InteractiveMap' modules in the workflow `FengPapesUpdate', displaying raw occurrence data and predicted distribution over a global map, allowing users to interactively explore their results. C) A response curve produced by the `ResponseCurve' module for the first covariate, bio1 in the workflow `FengPapesUpdate'."}
