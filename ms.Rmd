...
First, we describe the modular structure of zoon workflows, illustrate how to download and upload software modules, and demonstrate how completed `zoon` workflows can be shared, evaluated and extended.
Then we illustrate these concepts, and illustrate the contribution we hope `zoon` will make to SDM research, by recreating and extending two recent SDM analyses using `zoon` workflows.
...
Firstly, occurrence data must be read into the software package or downloaded from online repositories.
Occurrence usually takes the form of presence-absence data or presence-only data, though abundance data is also used.
Secondly, predictor variables or covariates (typically environmental covariates) are required and the values of these covariates, at the locations of the occurrence data, must be extracted.
Thirdly, a wide variety of processes are often applied to the occurrence and covariate data.
These processes include data cleaning, data thinning to account for spatial biases, feature selection using PCA or association tests and the splitting of data into training and test sets or cross validation folds.
Fourthly, once the data has been suitable manipulated a model is fitted to estimate the relationships between the covariates and occurrence data.
These models include simple statistical models such as GLMs as well as modern, flexible machine-learning methods such as MaxEnt and boosted regression trees.
Finally, model outputs are needed.
The predictive power and goodness of fit must be assessed and the model parameters or response curves must be examined.
The model is likely to be used to predict species occurrence, either in the vicinity of the occurrence data or elsewhere, or into the past or future.
...
#### Example 1. A fairly simple SDM, tweaking some different stuff and looking at the results
